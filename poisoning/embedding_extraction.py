...
def compute_embeddings(params, wav_scp, outdir):
    ...
    """
    with torch.no_grad():
        n_files = len(open(wav_scp, "r", encoding="utf-8").readlines())
        with open(wav_scp, "r", encoding="utf-8") as wav_scp:
            for line in tqdm.tqdm(wav_scp, total=n_files, desc="Extracting"):
